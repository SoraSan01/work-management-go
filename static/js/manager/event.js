document.addEventListener('DOMContentLoaded', function() {
  ...
  const setCreateMode = date => {
    ...
    form.action = '/manager/events/store';
  ...
  const setViewMode = event => {
    ...
    form.action = `/manager/events/update/${event.id}`;
    deleteForm.action = `/manager/events/delete/${event.id}`;
  ...
  // Fallback for manager layout-level click interference: drive toolbar actions explicitly.
  calendarEl?.addEventListener('click', (event) => {
    const button = event.target.closest(
      '.fc-prev-button, .fc-next-button, .fc-today-button, .fc-dayGridMonth-button, .fc-timeGridWeek-button, .fc-timeGridDay-button, .fc-listWeek-button'
    );
    if (!button) return;
    ...
    if (button.classList.contains('fc-prev-button')) calendar.prev();
    else if (button.classList.contains('fc-next-button')) calendar.next();
    else if (button.classList.contains('fc-today-button')) calendar.today();
    else if (button.classList.contains('fc-dayGridMonth-button')) calendar.changeView('dayGridMonth');
    else if (button.classList.contains('fc-timeGridWeek-button')) calendar.changeView('timeGridWeek');
    else if (button.classList.contains('fc-timeGridDay-button')) calendar.changeView('timeGridDay');
    else if (button.classList.contains('fc-listWeek-button')) calendar.changeView('listWeek');
  }, true);
