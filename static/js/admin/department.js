(() => {
    ...
    const info = document.getElementById('departmentsTableInfo');
    const pagination = document.getElementById('departmentsTablePagination');
...
(() => {
  ...
  const overlay = document.getElementById('addDepartmentModalOverlay');
  const openButtons = [
    document.getElementById('openAddDepartmentModal'),
    document.getElementById('openAddDepartmentModalEmpty')
  ].filter(Boolean);
  const closeButtons = [
    document.getElementById('closeAddDepartmentModal'),
    document.getElementById('cancelAddDepartmentModal')
  ].filter(Boolean);

  const form = document.getElementById('departmentModalForm');
  const title = document.getElementById('departmentModalTitle');
  const subtitle = document.getElementById('departmentModalSubtitle');
  const submitBtn = document.getElementById('departmentModalSubmit');

  const inputName = document.getElementById('departmentName');
  ...
  const setCreateMode = () => {
    ...
    if (subtitle) subtitle.textContent = 'Create a new Department.';
    if (submitBtn) submitBtn.textContent = 'Create Department';
    if (form) form.action = '/admin/departments/store';
    if (inputName) inputName.value = '';
  };

  const setEditMode = (data) => {
    if (title) title.textContent = 'Edit Department';
    if (subtitle) subtitle.textContent = 'Update department details.';
    if (submitBtn) submitBtn.textContent = 'Save Changes';
    if (form && data.editUrl) form.action = data.editUrl;
    if (inputName) inputName.value = data.departmentName || '';
  };
  ...
  document.querySelectorAll('[data-edit-department]').forEach(btn => {
    btn.addEventListener('click', () => {
      setEditMode({
        ...
        departmentName: btn.dataset.departmentName,
      });
      openModal();
    });
  });
